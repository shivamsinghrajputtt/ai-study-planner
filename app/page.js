...
export default function Home() {
  ...
  function downloadStudyPlan() {
    if (!studyPlan?.days?.length) return;
    ...
    const addWrapped = (text, fontSize = 10, gap = 5, bold = false) => {
      doc.setFont("helvetica", bold ? "bold" : "normal");
      doc.setFontSize(fontSize);
      const lines = doc.splitTextToSize(String(text || ""), contentWidth);
      if (y + lines.length * (fontSize * 0.42) + gap > pageHeight - 16) {
        doc.addPage();
        y = 18;
      }
    ...
    doc.setFontSize(20);
    doc.text("AI Study Planner", margin, y);
    y += 9;
    addWrapped((selectedSubject?.name || "Study Plan") + " - " + (selectedUnit?.name || ""), 13, 6, true);
    addWrapped("Exam date: " + examDate + " | Daily study time: " + studyPlan.dailyHours + " hours | Plan: " + studyPlan.daysAvailable + " days", 9, 6);
    addWrapped(studyPlan.summary, 10, 8);

    if (weakTopics.length > 0) {
      addWrapped("Weak topics to review:", 11, 3, true);
      weakTopics.forEach(({ topic, wrongCount }) => {
        addWrapped("- " + topic + " (" + wrongCount + " wrong)", 9, 2);
      });
      y += 3;
    }

    studyPlan.days.forEach((day) => {
      addWrapped("Day " + day.day + " - " + day.focus, 12, 2, true);
      addWrapped(day.date + " | " + day.durationMinutes + " minutes", 9, 3);
      addWrapped("Topics", 10, 2, true);
      (day.topics || []).forEach((topic) => addWrapped("- " + topic, 9, 2));
      addWrapped("Tasks", 10, 2, true);
      (day.tasks || []).forEach((task) => addWrapped("- " + task, 9, 2));
      y += 4;
    });

    doc.setFont("helvetica", "normal");
    doc.setFontSize(8);
    doc.text("Generated by AI Study Planner", margin, pageHeight - 8);
    doc.save("ai-study-plan-" + (examDate || "plan") + ".pdf");
  }
  ...
  return (
    <main className="relative min-h-screen overflow-hidden bg-slate-950 text-white">
      ...
      <section className="relative mx-auto max-w-5xl px-6 py-16 sm:py-24">
        ...
        <h1 className="animate-fade-up animation-delay-100 mt-4 max-w-4xl text-4xl font-bold tracking-tight sm:text-6xl">
          Turn your syllabus into a personalized study plan.
        </h1>

        <p className="animate-fade-up animation-delay-200 mt-6 max-w-2xl text-lg leading-8 text-slate-300">
